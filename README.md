...
The `Run pipeline` button on the dashboard re-runs the integrated backend workflow: demand forecasting, price recomputation, dashboard metric refresh and output-file update. It is included so an evaluator can demonstrate the system recalculating decisions after data/model changes instead of showing only static results.
